...
data::data(){
    database.push_back(people("","祝羽弦","炽凰"));
    database.push_back(people("","冥水鸢","沧冥"));
    database.push_back(people("","白永羲",""));
    database.push_back(people("","越千霜","霜虎"));
    database.push_back(people("","祝若笙","炽凰"));
    database.push_back(people("","白锦锦",""));
    database.push_back(clothes("春水送兰舟。冥水鸢离开南境时，祝羽弦一个人去了渡口。他在岸边占了很久，看冥家的渡船渐行渐远。",
                               "春水流*群青","祝羽弦","冥水鸢"));
    ...
    for(auto &iter:database){
        if(iter.isclothes()){
            ...
            if(temp->clothesName()==clothesName){
                temp->print_message((bool[6]){true,true,true,true,true,true});
            }
        }
    }
}
